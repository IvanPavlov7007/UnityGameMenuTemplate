...
public class Run
{
	...
	#region Run.After
	public static Run After(float aDelay, System.Action aAction, bool timeUnscaled = false)
	{
		var tmp = new Run();
		tmp.action = _RunAfter(tmp, aDelay, aAction, timeUnscaled);
		tmp.Start();
		return tmp;
	}
	private static IEnumerator _RunAfter(Run aRun, float aDelay, System.Action aAction, bool timeUnscaled)
	{
		aRun.isDone = false;
		if (timeUnscaled)
	...
	public static Run OnGUI(float aDuration, System.Action aAction, bool timeUnscaled = false)
	{
		var tmp = new Run();
		tmp.onGUIaction = aAction;
		if (aDuration > 0.0f)
			tmp.action = _RunAfter(tmp, aDuration, null, timeUnscaled);
		else
			tmp.action = null;
		tmp.Start();
